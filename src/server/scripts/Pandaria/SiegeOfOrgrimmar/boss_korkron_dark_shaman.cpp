...
class spell_korkron_dark_shaman_foul_stream : public SpellScriptLoader
{
    public:
        spell_korkron_dark_shaman_foul_stream() : SpellScriptLoader("spell_korkron_dark_shaman_foul_stream") { }

        class spell_korkron_dark_shaman_foul_stream_SpellScript : public SpellScript
        {
            PrepareSpellScript(spell_korkron_dark_shaman_foul_stream_SpellScript);
            ...
            void HandleBeforeCast()
            {
                ...
                baseTargetGuid = ObjectGuid::Empty;

                if (WorldObject* target = GetExplTargetUnit())
                {
                    baseTargetGuid = target->GetGUID();

                    // fix visual
                    GetCaster()->SetFacingToObject(target);
                    GetCaster()->SetOrientation(GetCaster()->GetAngle(target));
                }
            }
            ...
            void FilterTargets(std::list<WorldObject*>& targets)
            {
                ...
                Unit* baseTarget = ObjectAccessor::GetUnit(*caster, baseTargetGuid);
                if (!baseTarget)
                    return;
                ...
                for (std::list<WorldObject*>::iterator itr = targets.begin(); itr != targets.end();)
                {
                    ...
                    if (!(*itr)->IsInBetween(caster, baseTarget, 3.0f))
                    {
            ...
            void Register()
            {
                BeforeCast += SpellCastFn(spell_korkron_dark_shaman_foul_stream_SpellScript::HandleBeforeCast);
                OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(spell_korkron_dark_shaman_foul_stream_SpellScript::FilterTargets, EFFECT_0, TARGET_UNIT_CASTER_AREA_ENEMY);
                OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(spell_korkron_dark_shaman_foul_stream_SpellScript::FilterTargets, EFFECT_1, TARGET_UNIT_CASTER_AREA_ENEMY);
            }

        private:

            ObjectGuid baseTargetGuid;
        };

        SpellScript* GetSpellScript() const
        {
            return new spell_korkron_dark_shaman_foul_stream_SpellScript();
        }
};
...
void AddSC_boss_korkron_dark_shaman()
{
    ...
    new spell_korkron_dark_shaman_foul_stream();    // 144090
