/*
 * Copyright (C) 2019-2020 LatinCore <https://github.com/AshamaneProject>
 *
...
class spell_rook_stonetoe_shared_torment : public SpellScript
{
    PrepareSpellScript(spell_rook_stonetoe_shared_torment);

    void FilterTargets(std::list<WorldObject*>& p_Targets)
    {
        p_Targets.remove_if([&](WorldObject* p_Target) -> bool
        {
            return p_Target->GetEntry() != Adds::NPC_EMBODIED_MISERY && p_Target->GetEntry() != Adds::NPC_EMBODIED_SORROW && p_Target->GetEntry() != Adds::NPC_EMBODIED_GLOOM;
        });
    }

    void Register() override
    {
        OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(spell_rook_stonetoe_shared_torment::FilterTargets, EFFECT_0, TARGET_UNIT_SRC_AREA_ENTRY);
    }
};
...
void AddSC_boss_fallen_protectors()
{
    ...
    RegisterSpellScript(spell_rook_stonetoe_shared_torment);                 // 145655
}
